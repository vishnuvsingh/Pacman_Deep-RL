...
class DQN:
    def __init__(self, params):
        ...
        layer_name = 'conv0' ; size = 3 ; channels = 6 ; filters = 8 ; stride = 1
        self.w0 = tf.Variable(tf.random_normal([size,size,channels,filters], stddev=0.01),name=self.network_name + '_'+layer_name+'_weights')
        self.b0 = tf.Variable(tf.constant(0.1, shape=[filters]),name=self.network_name + '_'+layer_name+'_biases')
        self.c0 = tf.nn.conv2d(self.x, self.w0, strides=[1, stride, stride, 1], padding='SAME',name=self.network_name + '_'+layer_name+'_convs')
        self.bn0 = self.batch_norm(self.c0, 8, self.phase_train)
        self.o0 = tf.nn.leaky_relu(tf.add(self.bn0,self.b0),name=self.network_name + '_'+layer_name+'_activations')
        ...
        self.c1 = tf.nn.conv2d(self.o0, self.w1, strides=[1, stride, stride, 1], padding='SAME',name=self.network_name + '_'+layer_name+'_convs')
